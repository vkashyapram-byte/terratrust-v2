...
if (!PROD_URL || !SUPABASE_URL || !SUPABASE_KEY)
  throw new Error("PROD_URL, VITE_SUPABASE_URL, and VITE_SUPABASE_PUBLISHABLE_KEY are required");
...
async function runProductionQA() {
  ...
  const prodCitizen = await page.evaluate(() => {
    ...
    return {
      ...
      hasViolations: navLinks.some((h) =>
        ["/government", "/surveyor", "/bank", "/admin", "/community"].some((r) => h?.includes(r)),
      ),
    };
  });
  ...
  await new Promise((r) => setTimeout(r, 2000));
  const prodAccessDenied = await page.evaluate(() =>
  ...
  await new Promise((r) => setTimeout(r, 2000));
  const prodCommDecom = await page.evaluate(() =>
  ...
  await loginProd(page, "citizen");
  await page.goto(`${PROD_URL}/properties/TT-8421-BLR/verify`, {
  ...
  if (n8nCaptures.length > 0) {
    ...
    console.log(
      "[PROD] n8n Response Confidence Score:",
