...
async function main() {
  ...
  const aparsoftMounted = await page.evaluate(() => {
    return !!document.querySelector('[data-aparsoft-chatbot]') || !!document.querySelector('script[src*="aparsoft"]');
  });
  ...
  console.log('3. Real MapLibre mini-maps on property cards:', miniMapsCount, miniMapsCount >= 3 ? 'PASS' : 'FAIL');
  ...
  console.log('4. Detail page real MapLibre canvas present?:', report.detailMapLoaded ? 'PASS' : 'FAIL');
  ...
  const webhookPayload = {
    ...
    stateCode: 'KA',
    cadastralIdentifiers: { district: 'Bengaluru Urban', taluk: 'Bengaluru East', surveyNumber: '112/4' },
    property: {
      ...
      boundary: [
        ...
        { lat: 12.9868, lng: 77.7330 },
        { lat: 12.9856, lng: 77.7330 }
      ]
    }
  };
