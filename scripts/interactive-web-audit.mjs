...
async function runInteractiveAudit() {
  ...
  try {
    ...
    const hasArea =
      gisText.includes("Calculated Area") || gisText.includes("m²") || gisText.includes("sq ft");
    ...
    if (submitPropertyBtn) {
      ...
      console.log(
        "  -> Submitted property. Awaiting Supabase insert and live n8n 10-node response...",
      );
    ...
    if (passportBtn) {
      ...
      const hasPassportData =
        ...
        (passportPageText.includes("Trust") ||
          passportPageText.includes("Score") ||
          passportPageText.includes("Karnataka"));
      recordAudit(
        "Passport",
        true,
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "terratrust_demo_session",
        JSON.stringify({ id: "demo_surveyor", role: "surveyor", full_name: "Arjun Mehta" }),
      );
    });
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "terratrust_demo_session",
        JSON.stringify({ id: "demo_gov", role: "government", full_name: "Dr. Vandana Rao" }),
      );
    });
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "terratrust_demo_session",
        JSON.stringify({ id: "demo_comm", role: "community", full_name: "Rajendra Joshi" }),
      );
    });
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "terratrust_demo_session",
        JSON.stringify({ id: "demo_bank", role: "bank", full_name: "Sunita Sharma" }),
      );
    });
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "terratrust_demo_session",
        JSON.stringify({ id: "demo_admin", role: "admin", full_name: "System Admin" }),
      );
    });
