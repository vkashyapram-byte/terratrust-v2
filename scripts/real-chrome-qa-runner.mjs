...
const ARTIFACT_SCREENSHOTS_DIR =
  "/Users/kushal/.gemini/antigravity-ide/brain/293631ab-6085-43d5-a31a-ff87008ed9a4/screenshots";
...
async function runQA() {
  ...
  try {
    ...
    const hasKpis =
      dashText.includes("Registered Parcels") || dashText.includes("Portfolio Trust Score");
    ...
    if (notifLink) {
      ...
      await page.waitForFunction(() => window.location.pathname === "/notifications", {
        timeout: 6000,
      });
    ...
    if (!newPropBtn) {
      throw new Error(
        "Could not find Register Property / New Property Passport button on dashboard",
      );
    }
    ...
    await page.type(
      ...
      "Temporary QA property for TerraTrust browser verification.",
    );
    ...
    const stateOptions = await page.$$eval("#property-state-select option", (opts) =>
      opts.map((o) => o.value),
    );
    console.log(
      `States count: ${stateOptions.length}, includes Karnataka: ${stateOptions.includes("Karnataka")}`,
    );
    ...
    qaLog.steps["04-location"] = {
      status:
        countryVal === "India" && Math.abs(parseFloat(latVal) - 12.9716) < 0.05 ? "PASS" : "FAIL",
      country: countryVal,
      state: "Karnataka",
      city: "Bengaluru",
      lat: latVal,
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Placemark>
    ...
    console.log(
      `Review verification: Name=${hasName}, Type=${hasType}, Coords=${hasCoords}, Area=${hasArea}`,
    );
    ...
    if (passportId) {
      ...
      if (dbRecord) {
        console.log(
          `SUPABASE SUCCESS! Found record ID: ${dbRecord.id}, Status: ${dbRecord.status}, Trust: ${dbRecord.trust_score}`,
        );
    ...
    const reloadPersisted =
      afterReloadText.includes(passportId || "TT-") ||
      afterReloadText.includes("Test Citizen Property");
    ...
    if (myPropsNav) {
      ...
      await page.waitForFunction(() => window.location.pathname === "/properties", {
        timeout: 8000,
      });
      await new Promise((r) => setTimeout(r, 1200));
    }
    ...
    const hasNewPropInList =
      myPropsText.includes("Test Citizen Property") ||
      (passportId && myPropsText.includes(passportId));
