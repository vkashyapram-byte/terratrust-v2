...
async function runGisSuite() {
  ...
  try {
    ...
    record('Location Step', 'Current Location Button Exists', step2CurrentLocBtn ? 'PASS' : 'FAIL', 'Found #btn-step2-current-location');
    ...
    record('Real Map', 'Map Container Rendered', canvas ? 'PASS' : 'FAIL', 'Found #gis-boundary-canvas');
    ...
    record('Real Map', 'MapLibre WebGL Canvas Initialized', isMapLibreLoaded ? 'PASS' : 'FAIL', 'WebGL canvas element created');
    ...
    record(
      ...
      attributionText.includes('OpenStreetMap') || attributionText.includes('CARTO') ? 'PASS' : 'FAIL',
    ...
    if (mapLocBtn) {
      ...
      const capturedGps = textAfterLoc.includes('Captured device coordinates') || textAfterLoc.includes('Selected Position:');
      record('Real Map', 'Fly To Device Location', capturedGps ? 'PASS' : 'FAIL', 'Device geolocation triggered map flyTo');
    }
    ...
    if (dropdownCount > 0) {
      ...
      record(
        ...
        textAfterSelect.includes('Centered map on:') || textAfterSelect.includes('Selected Position:') ? 'PASS' : 'FAIL',
    ...
    if (drawBtn) {
      await drawBtn.click();
      await new Promise(r => setTimeout(r, 400));
      const isDrawingActive = await page.evaluate(() => document.body.innerText.includes('Drawing Mode'));
      record('Boundary Drawing', 'Toggle Draw Mode', isDrawingActive ? 'PASS' : 'FAIL', 'Active drawing banner rendered');

      // Click on canvas to add vertex
      const canvasBox = await canvas.boundingBox();
      if (canvasBox) {
        await page.mouse.click(canvasBox.x + canvasBox.width / 2 + 50, canvasBox.y + canvasBox.height / 2 + 50);
        await new Promise(r => setTimeout(r, 600));
      }
    ...
    if (geoJsonInput) {
      ...
      const geoJsonSuccess = pageText.includes('Successfully imported GeoJSON') || pageText.includes('4 Vertices');
      record('GeoJSON Import', 'Import Valid RFC 7946 Polygon', geoJsonSuccess ? 'PASS' : 'FAIL', 'Loaded GeoJSON coordinates and fitted bounds');
    }
    try { fs.unlinkSync(tmpGeoJsonPath); } catch {}
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Placemark>
    ...
    if (kmlInput) {
      ...
      await new Promise(r => setTimeout(r, 800));
      const pageText = await page.evaluate(() => document.body.innerText);
      const kmlSuccess = pageText.includes('Successfully imported KML') || pageText.includes('4 Vertices');
      record('KML Import', 'Import Google Earth KML Polygon', kmlSuccess ? 'PASS' : 'FAIL', 'Converted KML to GeoJSON and rendered polygon');
    }
    try { fs.unlinkSync(tmpKmlPath); } catch {}
    ...
    const hasGpsInReview = reviewText.includes('Center GPS:') && reviewText.includes('GIS Polygon Boundary');
    const hasAreaInReview = reviewText.includes('Calculated Area:') && reviewText.includes('acres');
    record('Review Step', 'Real GPS & Area In Summary', hasGpsInReview && hasAreaInReview ? 'PASS' : 'FAIL', 'Review step displays real coordinates, boundary metrics, and acres');
    ...
    record('Supabase Persistence', 'Passport Created & Persisted', 'PASS', `Passport ID: ${passportId}`);

    // Open Property Detail Page to confirm real geometry reload
    const propLink = await page.$('a[href*="/properties/"]');
    if (propLink) {
      await propLink.click();
      await page.waitForFunction(() => window.location.pathname.startsWith('/properties/'), { timeout: 8000 });
      await new Promise(r => setTimeout(r, 1000));
    }
