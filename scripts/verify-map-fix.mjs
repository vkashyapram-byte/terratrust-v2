...
async function verifyMap() {
  ...
  // Check canvas exists
  const canvasExists = await page.evaluate(() => !!document.querySelector('canvas.maplibregl-canvas'));
  ...
  await page.goto(`${BASE_URL}/properties/p_001/gis-layers`, { waitUntil: 'domcontentloaded', timeout: 30000 });
  await new Promise((r) => setTimeout(r, 3000));
  ...
  fs.writeFileSync(path.join(SCREENSHOTS_DIR, 'map-qa-report.json'), JSON.stringify(report, null, 2));
