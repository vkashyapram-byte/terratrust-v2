    ...
    <AppShell title="Settings" subtitle="Preferences, security, and integrations.">
      <div className="grid gap-6">
        {[
          {
            ...
            rows: [
              [
                ...
                "Email me when a property's verification status changes.",
                true,
              ],
              [
                "Community attestations",
                "Notify me when neighbors attest a property I track.",
                true,
              ],
            ...
            <div className="grid gap-5">
              {section.rows.map(([t, d, on]) => (
                <div key={t as string} className="flex items-start justify-between gap-6">
                  ...
                  <Switch
                    checked={preferences[t as string] ?? (on as boolean)}
                    onCheckedChange={(checked) =>
                      setPreferences((current) => ({ ...current, [t as string]: checked }))
                    }
                    aria-label={`Toggle ${t as string}`}
                  />
                </div>
              ))}
            </div>
        ...
        <Button
          ...
          onClick={() =>
            toast.error(
              "Preference persistence requires the authenticated profiles service, which is not configured in this build.",
            )
          }
        >
          Save preferences
