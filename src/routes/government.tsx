      ...
      actions={
        <>
          <Button
            ...
            onClick={() =>
              downloadTextFile(
                ...
                "terratrust-regional-verification-report.json",
                "application/json",
              )
            }
          >
          ...
          <Button
            ...
            onClick={() =>
              toast.error(
                "Bulk registry ingestion requires the authenticated government registry service, which is not configured in this build.",
              )
            }
          >
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_400px]">
        ...
        <div className="surface-card flex flex-col p-5">
          <p className="font-medium flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 text-destructive" /> Dispute queue
          </p>
          <ul className="mt-3 divide-y divide-border">
            {properties
              .filter((p) => p.status === "disputed")
              .concat(properties.slice(1, 2))
              .map((p) => (
                <li key={p.id} className="py-3">
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        toast.error(
                          "Dispute resolution requires the authenticated disputes service, which is not configured in this build.",
                        )
                      }
                    >
                      Resolve
