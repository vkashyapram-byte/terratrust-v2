    ...
    <AppShell title="Regions" subtitle="Coverage and verification health per state.">
      ...
      <div className="mt-6">
        <DataTable rows={rows} columns={[
          { key: "name", label: "Region", render: r => <span className="font-medium">{r.name}</span> },
          ...
          { key: "rate", label: "Rate", render: r => <Pill tone={parseFloat(r.rate) > 90 ? "success" : "warning"}>{r.rate}</Pill> },
        ]} />
      </div>
    </AppShell>
  );
