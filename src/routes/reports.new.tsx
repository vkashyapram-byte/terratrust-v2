      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <Field label="Report type">
            <div className="flex flex-wrap gap-2">
              {kinds.map((k, i) => (
                <button
                  key={k}
                  className={`rounded-full px-3 py-1.5 text-xs ring-1 ${i === 0 ? "bg-primary text-primary-foreground ring-primary" : "bg-surface ring-border hover:bg-muted"}`}
                >
                  {k}
                </button>
              ))}
        ...
        <div className="surface-card flex flex-col gap-3 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            4 properties · ₹8.85 Cr total estimated value · 3 verified, 1 pending
          </p>
