...
function FraudPage() {
  return (
    <AppShell
      ...
      subtitle="AI-flagged anomalies across ownership, boundaries, and documents."
      actions={
        <Button
          variant="outline"
          ...
          onClick={() =>
            downloadTextFile(
              [
                "Case,Parcel,Region,Risk,Anomaly,Status",
                ...cases.map(
                  (item) =>
                    `${item.id},${item.parcel},${item.region},${item.risk},${item.kind},${item.status}`,
                ),
              ].join("\n"),
              "terratrust-fraud-cases.csv",
              "text/csv",
            )
          }
        >
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              key: "flagged",
              label: "Flagged",
              render: (r) => <span className="text-muted-foreground">{r.flagged}</span>,
            },
            {
              key: "status",
              label: "Status",
