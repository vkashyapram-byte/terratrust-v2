      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        <div className="surface-card flex flex-col items-center p-6">
          <ScoreRing
            value={78}
            label="Land Health"
            sublabel="Healthy · agri-suitable"
            tone="success"
          />
          <div className="mt-5 grid w-full grid-cols-2 gap-3">
            <SignalTile
              ...
              label="NDVI"
              value={landHealth.ndvi.toFixed(2)}
              tone="success"
            />
            <SignalTile
              icon={<Droplets className="h-3 w-3 text-info" />}
              label="Moisture"
              value={landHealth.moisture.toFixed(2)}
              tone="success"
            />
            <SignalTile
              icon={<Mountain className="h-3 w-3 text-warning-foreground" />}
              label="Slope"
              value={`${landHealth.slope}°`}
              tone="success"
            />
            <SignalTile
              icon={<Sprout className="h-3 w-3 text-success" />}
              label="Soil C"
              value={`${landHealth.soilCarbon}%`}
              tone="success"
            />
            <SignalTile
              icon={<TreePine className="h-3 w-3 text-success" />}
              label="Tree cvr"
              value={`${(landHealth.treeCover * 100).toFixed(0)}%`}
              tone="default"
            />
            <SignalTile
              label="Erosion"
              value={`${(landHealth.erosion * 100).toFixed(0)}%`}
              tone="success"
            />
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            <AIInsightCard
              ...
              value="+0.06"
              delta={{ value: 9, label: "YoY NDVI" }}
              tone="success"
            />
            <AIInsightCard
              title="Water stress"
              value="Low"
              hint="Moisture above seasonal mean"
              tone="success"
            />
            <AIInsightCard
              title="Climate exposure"
              value="Moderate"
              hint="2 heat events this season"
              tone="warning"
            />
          </div>
          ...
          <div className="surface-card p-6">
            <SectionTitle
              eyebrow="12 months"
              title="NDVI & soil moisture"
              description="Sentinel-2 derived, smoothed monthly."
            />
            <div className="h-64">
              <ResponsiveContainer>
                <AreaChart data={ndviSeries}>
                  <defs>
                    <linearGradient id="ndvi" x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.72 0.16 145)" stopOpacity={0.5} />
                      <stop offset="100%" stopColor="oklch(0.72 0.16 145)" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="moist" x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor="oklch(0.55 0.1 180)" stopOpacity={0.4} />
          ...
          <div className="surface-card p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <ConfidenceMeter
                value={Math.round(landHealth.ndvi * 100)}
                label="Vegetation density (NDVI)"
              />
              <ConfidenceMeter
                value={Math.round(landHealth.moisture * 100)}
                label="Soil moisture"
              />
              <ConfidenceMeter
                value={Math.round((1 - landHealth.erosion) * 100)}
                label="Erosion resistance"
              />
              <ConfidenceMeter
                value={84}
                label="Slope suitability"
                hint="4.2° — ideal for residential"
              />
