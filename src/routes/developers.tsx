...
export const Route = createFileRoute("/developers")({
  head: () => ({
    meta: [
      { title: "Developers — TerraTrust AI" },
      {
        name: "description",
        content: "REST & webhook APIs to integrate Property Passports anywhere.",
      },
    ],
  }),
  component: Page,
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-6 py-20">
        ...
        <p className="mt-3 max-w-xl text-muted-foreground">
          Stable REST APIs, webhooks, and SDKs in TypeScript, Python and Go.
        </p>
          ...
          {[
            {
              icon: BookOpen,
              t: "REST API",
              d: "Read passports, file disputes, submit boundaries.",
            },
            {
              icon: Webhook,
              t: "Webhooks",
              d: "Real-time events for verifications, disputes, and AI runs.",
            },
            {
              icon: KeyRound,
              t: "OAuth & API keys",
              d: "Granular scopes per app and per environment.",
            },
        ...
        <div className="mt-12 surface-card overflow-hidden">
          <div className="border-b border-border bg-muted/40 px-4 py-2 text-xs font-mono">
            examples/fetch-passport.ts
          </div>
        ...
        <p className="mt-2 max-w-2xl text-muted-foreground">
          TerraTrust posts a verification payload to your n8n webhook (
          <span className="font-mono text-xs">VITE_N8N_WEBHOOK_URL</span>) and renders the returned
          decision. The URL is public config, not a secret; signing keys stay server-side. Without a
          configured webhook the app runs a deterministic local simulation of the same graph.
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div className="surface-card overflow-hidden">
            <div className="border-b border-border bg-muted/40 px-4 py-2 text-xs font-mono">
              request · POST /webhook/terratrust/verify
            </div>
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          Importable workflow:{" "}
          <span className="font-mono text-xs">n8n/terratrust-verification-workflow.json</span> — 10
          nodes, with a human-in-the-loop gate that escalates low-confidence or elevated-fraud
          parcels to the government review queue instead of auto-approving.
        </p>
