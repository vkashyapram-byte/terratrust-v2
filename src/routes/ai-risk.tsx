...
const factors = riskBreakdown.map((r) => ({
  ...
  note:
    r.value > 30
      ? "Elevated — model recommends mitigating action."
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-4">
        <AIInsightCard
          icon={<Activity className="h-3 w-3 text-success" />}
        ...
        <AIInsightCard
          ...
          value="Market"
          hint="34 · macro FX volatility"
          tone="warning"
        />
        <AIInsightCard
          title="Lowest dimension"
          value="Fraud"
          hint="9 · no signals detected"
          tone="success"
        />
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
        <div className="surface-card flex flex-col items-center p-6">
          ...
          <p className="mt-4 text-center text-xs text-muted-foreground">
            Calibrated against 2,400 historical disputed parcels.
          </p>
        ...
        <div className="surface-card p-5">
            ...
            {riskBreakdown.map((r) => (
              <div key={r.label}>
                <ConfidenceMeter
                  value={r.value}
                  label={r.label}
                  hint={r.value > 30 ? "Mitigation recommended" : "Within tolerance"}
                />
              </div>
            ))}
          </div>
          <div className="mt-5 rounded-xl bg-warning/10 p-4 ring-1 ring-warning/30">
            <p className="flex items-center gap-2 text-sm font-medium">
              <ShieldAlert className="h-4 w-4 text-warning-foreground" /> Recommended next action
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Hedge market-risk exposure by re-running valuation in 7 days. Re-survey boundary to
              reduce overlap exposure on TT-7710-BLR.
            </p>
          </div>
        </div>
      </div>
