...
function Page() {
  ...
  return (
    <AppShell title="Bank origination" subtitle="Underwriting view of Property Passports shared with your institution.">
      <KpiRow items={[
      ...
      <div className="mt-3">
        <DataTable rows={pipeline} columns={[
          { key: "id", label: "Application", render: r => <span className="font-mono text-xs">{r.id}</span> },
          ...
          { key: "t", label: "Trust", render: r => <Pill tone={r.trust > 85 ? "success" : r.trust > 65 ? "warning" : "danger"}>{r.trust}</Pill> },
          { key: "d", label: "Decision", render: r => <Pill tone={r.decision === "Approved" ? "success" : r.decision === "Review" ? "warning" : "danger"}>{r.decision}</Pill> },
        ]} />
      </div>
    </AppShell>
  );
