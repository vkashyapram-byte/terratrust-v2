...
export const Route = createFileRoute("/integrations")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Connect TerraTrust to registries, banks, and the n8n verification orchestrator.",
      },
    ],
  }),
  component: Page,
...
const integrations = [
  {
    icon: Database,
    name: "Karnataka Bhoomi Registry",
  ...
  {
    icon: Building2,
    name: "BBMP Town Planning / Survey",
  ...
  },
  {
    ...
    desc: "Sync roads and POI within 5km of parcels.",
    connected: true,
  },
  {
    icon: MessageSquare,
    name: "WhatsApp Business Alerts",
    desc: "Notifications for verification milestones.",
    connected: false,
  },
    ...
    <div className="surface-card p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Automates document verification, fraud checks, boundary analysis, risk scoring,
              confidence calculation, and passport readiness.
            </p>
          </div>
        </div>
        {provider === "n8n" ? (
      ...
      <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-3">
        <div className="rounded-xl border border-border bg-muted/20 p-4">
          ...
          <dd className="mt-1 font-medium">
            {provider === "n8n" ? `Configured · ${host}` : "Not configured"}
          </dd>
          <p className="mt-1 text-xs text-muted-foreground">
            Set <span className="font-mono">VITE_N8N_WEBHOOK_URL</span> to go live. No secrets are
        ...
        <div className="rounded-xl border border-border bg-muted/20 p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            OCR → Fraud → Boundary → Risk → Confidence → Decision gate.
          </p>
        </div>
        <div className="rounded-xl border border-border bg-muted/20 p-4">
          ...
          <dd className="mt-1 font-medium">
            {provider === "n8n" ? "Awaiting first live execution" : "Local simulation only"}
          </dd>
          <p className="mt-1 text-xs text-muted-foreground">
            Every run emits workflowId, timestamp, scores and executed steps.
          </p>
        </div>
      </dl>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button asChild className="rounded-full">
          <Link to="/properties/$id/verify" params={{ id: "c63deb92-22df-4cb7-903c-25866f7d6aa0" }}>
            Test workflow · TT-8421-BLR
          </Link>
        </Button>
        <Button asChild variant="outline" className="rounded-full">
          <Link to="/properties/$id/verify" params={{ id: "21d50f16-bbbe-4bf2-be13-51fb000ae922" }}>
            Human-review path · TT-4628-KA
          </Link>
        </Button>
        <Button asChild variant="outline" className="rounded-full">
          <Link to="/developers">Webhook contract</Link>
        </Button>
      </div>

      <div className="mt-5 overflow-hidden rounded-xl border border-border">
        <div className="border-b border-border bg-muted/40 px-4 py-2 font-mono text-xs">
          Expected response · POST $VITE_N8N_WEBHOOK_URL
        </div>
...
function Page() {
  return (
    <AppShell
      title="Integrations"
      subtitle="Connect TerraTrust to the registries and tools you already use."
    >
      <Orchestrator />
      <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {integrations.map((i) => (
          <div key={i.name} className="surface-card p-5">
            ...
            <Button variant={i.connected ? "outline" : "default"} className="mt-4 w-full">
