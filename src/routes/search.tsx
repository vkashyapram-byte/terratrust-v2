    ...
    >
      <div className="surface-card p-5">
        <div className="flex gap-2">
          ...
          <Button type="button" className="h-12" onClick={() => setMode("auto")}>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {filters.map((x) => (
            <button
              key={x}
              onClick={() => setF(x)}
              className={`rounded-full px-3 py-1 text-xs ring-1 transition ${f === x ? "bg-primary text-primary-foreground ring-primary" : "bg-surface text-muted-foreground ring-border hover:bg-muted"}`}
            >
              {x}
            </button>
          ))}
      ...
      <p className="mt-5 text-xs text-muted-foreground">
        Showing {results.length} of 2,418,332 parcels —{" "}
        {q.trim() ? "ranked by relevance" : "all matches"} · mode{" "}
        <span className="font-medium text-foreground">{modeMeta.label.toLowerCase()}</span>
      </p>
      ...
      <div className="mt-3 space-y-3">
        {results.map((p) => (
          <Link
            ...
            to="/properties/$id"
            params={{ id: p.id }}
            className="surface-card flex items-center gap-4 p-4 hover:shadow-[var(--shadow-elev)] transition"
          >
            <div className="grid h-16 w-20 place-items-center rounded-lg bg-gradient-to-br from-primary/10 to-accent/10 font-mono text-[10px] text-muted-foreground">
              {p.passportId}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                Owner: {p.owner} · {p.coords.lat.toFixed(4)}, {p.coords.lng.toFixed(4)}
              </p>
            </div>
