    ...
    <div className="surface-card p-5">
      ...
      {items.length === 0 ? (
        <p className="mt-3 text-sm text-muted-foreground">
          No active encumbrances. Passport is free of liens, mortgages, and caveats.
        </p>
      ) : (
...
function StatusDot({ status }: { status: Encumbrance["status"] }) {
  const c =
    status === "active" ? "bg-warning" : status === "contested" ? "bg-destructive" : "bg-success";
          ...
          return (
            ...
            >
              ...
              <div className="text-right">
                ...
                <p
                  className={`text-[10px] ${i.impact === "positive" ? "text-success" : i.impact === "negative" ? "text-destructive" : "text-muted-foreground"}`}
                >
                  {i.impact}
                </p>
              </div>
          ...
          return (
            <div key={r.key} className="rounded-lg border border-border bg-surface p-3">
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`ml-auto rounded-full px-2 py-0.5 text-[10px] capitalize ring-1 ${r.severity === "high" ? "bg-destructive/10 text-destructive ring-destructive/20" : r.severity === "moderate" ? "bg-warning/15 text-warning-foreground ring-warning/30" : "bg-success/10 text-success ring-success/20"}`}
                >
                  {r.severity}
                </span>
