    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
        ...
        {result && (
          <>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <SignalTile icon={<ShieldAlert className="h-3.5 w-3.5" />} label="Fraud screening" value={`${result.fraudBand ?? "Pending"} · ${result.fraudScore ?? 0}`} tone={(result.fraudScore ?? 0) >= 50 ? "danger" : (result.fraudScore ?? 0) >= 25 ? "warning" : "success"} />
              <SignalTile icon={<Activity className="h-3.5 w-3.5" />} label="Composite risk" value={`${result.riskScore ?? 0}/100`} tone={(result.riskScore ?? 0) >= 45 ? "warning" : "success"} />
            </div>
      ...
      <div className="surface-card flex flex-col items-center p-5">
        ...
        <div className="mt-6 w-full space-y-3">
          ...
          <ConfidenceMeter value={result?.fraudScore != null ? 100 - result.fraudScore : 0} label="Fraud clearance" />
          <ConfidenceMeter value={result?.riskScore != null ? 100 - result.riskScore : 0} label="Risk clearance" />
