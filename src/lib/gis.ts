...
export function propertyBoundaryFeature(property: Property): BoundaryFeature {
  ...
  if (coordinates.length < 3) {
    return feature(
      ...
      { propertyId: property.id, source: "registered" },
    );
  }
  const first = coordinates[0];
  const ring =
    coordinates.at(-1)?.[0] === first[0] && coordinates.at(-1)?.[1] === first[1]
      ? coordinates
      : [...coordinates, first];
  return polygon([ring], { propertyId: property.id, source: "registered" });
}

export function demoBoundaryFeatures(property: Property): {
  ...
  const coordinates =
    registeredBoundary.geometry.type === "Polygon"
      ? (registeredBoundary.geometry.coordinates[0] ?? [])
      : [];
  ...
  const submittedBoundary =
    submittedCoordinates.length >= 4
...
export function analyzeBoundaries(
  ...
  if (!registeredBoundary || !submittedBoundary) {
    conflicts.push({
      ...
      severity: "high",
      message: "Both registered and submitted boundaries are required.",
    });
  ...
  if (!isValidBoundary(registeredBoundary) || !isValidBoundary(submittedBoundary)) {
    conflicts.push({
      type: "invalid_geometry",
      severity: "high",
      message: "One or more polygons are invalid or self-intersecting.",
    });
    return { boundaryScore: 0, overlapPercentage: 0, boundaryVerified: false, conflicts };
  }
  ...
  const centroidDisplacementMeters = distance(
    centroid(registeredBoundary),
    centroid(submittedBoundary),
    { units: "meters" },
  );
  const areaAgreement = Math.max(
    0,
    100 - (Math.abs(registeredArea - submittedArea) / registeredArea) * 100,
  );
  const displacementScore = Math.max(
    0,
    100 - (centroidDisplacementMeters / GIS_CONFIG.boundaryToleranceMeters) * 100,
  );
  const boundaryScore = Math.round(
    Math.max(
      0,
      Math.min(100, overlapPercentage * 0.55 + areaAgreement * 0.25 + displacementScore * 0.2),
    ),
  );
  ...
  if (overlapPercentage < GIS_CONFIG.minimumOverlapPercentage) {
    conflicts.push({
      type: "overlap",
      severity: overlapPercentage < 50 ? "high" : "medium",
      message: `Only ${overlapPercentage.toFixed(1)}% of the submitted boundary overlaps the registered parcel.`,
    });
  }
  if (centroidDisplacementMeters > GIS_CONFIG.boundaryToleranceMeters) {
    conflicts.push({
      type: "displacement",
      severity: "high",
      message: `Boundary centroid is displaced by ${centroidDisplacementMeters.toFixed(1)}m.`,
    });
  }
  if (areaAgreement < 90) {
    conflicts.push({
      type: "area_difference",
      severity: areaAgreement < 70 ? "high" : "medium",
      message: "Submitted and registered parcel areas differ materially.",
    });
  }
...
function isValidBoundary(boundary: BoundaryFeature) {
  if (boundary.geometry.type === "Polygon" && boundary.geometry.coordinates[0]?.length >= 4)
    return booleanValid(boundary);
  if (boundary.geometry.type === "MultiPolygon" && boundary.geometry.coordinates.length > 0)
    return booleanValid(boundary);
  return false;
}
...
export function boundaryFromCoordinates(
