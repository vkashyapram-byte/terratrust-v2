...
export function getFraudReport(p: Property): FraudReport {
  ...
  if (p.status === "disputed") {
    signals.push({
      ...
      evidence: [
        "AI polygon diff · 0.84 IoU",
        "Bureau cadastre overlay",
        "Adjacent owner objection on file",
      ],
    });
    signals.push({
      id: "f2",
  ...
  if (p.documents.length < 2)
    signals.push({
      ...
      detail:
        "Only 1 supporting document on file — increases susceptibility to fabricated filings.",
      evidence: ["Document count below median (3)"],
    });

  const riskScore = Math.min(
    100,
    Math.round(signals.reduce((a, x) => a + sevWeight[x.severity], 0)),
  );
  ...
  const summary =
    band === "Critical"
      ? "Critical fraud signals — manual bureau review required before any transaction."
      : band === "Elevated"
        ? "Elevated risk — recommend surveyor re-measurement and bureau cross-check."
        : band === "Watch"
          ? "Low-grade anomalies present; passport remains transactable."
