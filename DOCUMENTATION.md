...
| Deployment | Vite / Vercel / Cloudflare standalone build |
...
- **Secrets:** all keys via Supabase / Environment secret store — never in code.
...
flowchart TB
  ...
  subgraph Cloud[Supabase & n8n Cloud]
    DB[(Postgres + RLS)]
    Store[(Storage)]
    Auth[Auth]
  end
...
### 22.4 Production
Vite / Vercel / Cloudflare standalone build → Cloudflare Workers edge. Custom domain via project settings.
...
48. **How are secrets managed?** Supabase / Environment secret store.
...
- **Secrets:** managed through Supabase / Environment secret store — never in code, never in the client.
